...
cat > "$USB/CouncilKey-Os/.pendrive.env" <<EOF
# generated by pendrive-setup.sh - do not edit unless you know why
export COUNCIL_HOME="$USB/council-data"
...
rem 2. load the stick config (COUNCIL_HOME -> stick)
for /f "usebackq delims=" %%L in ("%ROOT%\.pendrive.env") do set "%%L"

rem 3. start the dashboard
echo.
echo   Dashboard: http://localhost:8443   (Ctrl+C to stop)
echo.
...
set -euo pipefail
ROOT="$(cd "$(dirname "$0")" && pwd)/CouncilKey-Os"
export COUNCIL_PENDRIVE=1
[ -f "$ROOT/.pendrive.env" ] && . "$ROOT/.pendrive.env"
