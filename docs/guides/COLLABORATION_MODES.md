...
User: "Build me a website"
  ...
  └── Agent Zero Builder (Code & Transparency): Writes code in work_dir, creates own tool if needed, uses solutions/build-website/ memorized
  ↓ Collect 3 responses
...
- Transparency: Agent Zero writes code inspectable
...
- Agent-specific features: Hermes TUI multiline editing, OpenClaw WhatsApp bridge, Agent Zero Canvas full Linux desktop
...
council ask --mode alone --agent agent-zero "Write code for website in work_dir"
...
User: "Ok, let Hermes alone design, then council vote"
  ...
  -> Together: Council votes on Hermes design, OpenClaw executes, Agent Zero writes code
```
...
1. **All Binaries on Pendrive, Not Host:**
   ...
   - `tools/linux/agent-zero/.venv/` - Agent Zero venv on USB
...
2. **All Configs + Caches + Temp Redirected to Pendrive (Key Trick):**
   ...
   export AGENT_ZERO_HOME="$USB/config/council/agent-zero/real_home"
...
5. **Data Stored In Pendrive:**
   ...
   - `config/council/agent-zero/keep/` - settings.json, knowledge/custom/, solutions/
...
- Creates venvs for hermes, agent-zero, council-core on USB
...
**You can switch:** Together for safety, alone for speed or agent-specific features (Hermes TUI, OpenClaw WhatsApp, Agent Zero Canvas)
...
- Portable: `USB/config/council/hermes/keep/` SOUL.md MEMORY.md USER.md skills/ etc. + `openclaw/keep/` + `agent-zero/keep/` + `shared/memory.md` + `journal/*.md` git + `secrets/` GPG encrypted
...
- Update council/orchestrator/main.py to support --mode together|alone --agent hermes|openclaw|agent-zero
