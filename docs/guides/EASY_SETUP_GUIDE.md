...
| Agent | On the stick |
|---|---|
| Council (Hermes/OpenClaw/OpenCode roles) | answers via your API key - no install at all |
...
| **OpenCode** | `RUN-OPENCODE.bat` (npm CLI on the stick - local execution, no Docker) |
...
|---|---|---|
| **Agent menu** | double-click `AGENTS.bat` (or `bash agents-menu.sh`) | menu: **A = ALL agents + dashboard**, or pick 1–6 (dashboard / OpenClaw / Hermes / CrewAI / Aider / OpenCode) |
...
| **[3/5]** External agents | **pick which agents** (comma-separated, e.g. `2,4`): 1 Hermes (5-15 min) · 2 OpenClaw (1-3 min) · 3 OpenCode (1-3 min, npm, no Docker) · 4 CrewAI (2-5 min) · 5 Aider (1-2 min) · 0 none. CrewAI + Aider install together in one pip command |
...
| OpenCode | npm `opencode-ai` - local execution, **no Docker** |
...
| builder & review | OpenCode | same provider |
...
Hermes, OpenClaw and OpenCode are interactive chat agents with their own
...
`COUNCIL_HERMES_URL` / `COUNCIL_OPENCLAW_URL` / `COUNCIL_OPENCODE_URL` and
...
> installer, and OpenCode ships as an npm package. Official installers are the
...
== Council: together (majority) ==
   ...
   - opencode        openai (gpt-4o-mini)           0.4s
   consensus: ✅ 3/3

# Council Decision - Consensus 3/3 ✅
## hermes (council-role) ... ## openclaw (council-role) ... ## opencode ...
```
Variants:
```bash
...
OpenClaw=execution, OpenCode=review) answers independently with its own
