...
def test_current_version_reads_v1_1():
    ...
    assert current_version().startswith("1.20")
...
def _responses() -> list:
    ...
    return [
        ...
        VoteResult("opencode", "builder", "danger: rm -rf / is unsafe", "reject", 0.9),
    ]
...
def test_run_council_vote_shape():
    ...
    responses = [
        ...
        {"agent": "opencode", "role": "builder", "response": "danger", "status": "offline (mock)"},
    ]
...
def test_api_health_version_system_metrics():
    ...
    assert client.get("/api/version").json()["version"].startswith("1.20")
...
def test_cli_console_script_installed():
    ...
    assert out.stdout.strip().splitlines()[0] == "1.20.0"
