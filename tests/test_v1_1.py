...
def test_current_version_reads_v1_1():
    ...
    assert current_version().startswith("1.12")
...
def test_api_health_version_system_metrics():
    ...
    assert client.get("/api/version").json()["version"].startswith("1.12")
...
def test_cli_console_script_installed():
    ...
    assert out.stdout.strip() == "1.12.0"
